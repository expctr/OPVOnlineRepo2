/*
 * ¬ данном файле содержитс€ реализаци€ класса, который реализует алгоритм кластеризации на основе минимального
 * остовного дерева (при котором из него удал€етс€ N самых т€желых ребер).
 */
...
MinimumSpanningTreeClusteringUtil::MinimumSpanningTreeClusteringUtil(
...
std::vector<std::vector<size_t>> MinimumSpanningTreeClusteringUtil::splitGraph(
	...
	for (size_t i = 0; i < clustersNumber - 1 && i < edges.size(); ++i) {
		std::cout << "splitGraph next cycle. i: " << i << " from " << clustersNumber - 1 << "\n";
...
void MinimumSpanningTreeClusteringUtil::setClusteringObjects(
	std::vector<ClusteringObject*>* clusteringObjects
) {
	graphClusteringUtil->setClusteringObjects(clusteringObjects);
}

void MinimumSpanningTreeClusteringUtil::getClusters(
...
std::vector<std::vector<size_t>> MinimumSpanningTreeClusteringUtil::getComponents(
	std::vector<std::vector<size_t>>& graph
) {
	std::vector<std::vector<size_t>> result;
