...
class Segment {
	 ...
	 */
	private firstNeighbour: Segment | null = null

	/**
	 * Второй соседний отрезок.
	 */
	private secondNeighbour: Segment | null = null
	 ...
	 */
	public firstEndNeighbours: Segment[] = []

	/**
	 * Список соседей по второму концу.
	 */
	public secondEndNeighbours: Segment[] = []
	 ...
	 */
	public constructor(firstEnd: Point, secondEnd: Point, orientation: SegmentOrientation | null) {
	 ...
	 */
	public getFirstNeighbour(): Segment | null {
	 ...
	 * @returns упомянутый отрезок.
	 */
	public getSecondNeighbour(): Segment | null {
	 ...
	 */
	public isNewNeighbour(segment: Segment): boolean {
	 ...
	 * @returns true, если упомянутое утверждение истинно. Иначе - false.
	 */
	public isFirstNeighbour(segment: Segment): boolean {
	 ...
	 * @returns true, если упомянутое утверждение истинно. Иначе - false.
	 */
	public isSecondNeighbour(segment: Segment): boolean {
		if (this.firstEnd.approximatelyEquals(segment.firstEnd)
			&& this.secondEnd.approximatelyEquals(segment.secondEnd)) {
			return false;
		} else if (this.firstEnd.approximatelyEquals(segment.secondEnd)
	 ...
	 * @param segment указанный отрезок.
	 */
	public connectAsNeighbour(segment: Segment): void {
	 ...
	 */
	private getFirstSameOrientedNeighbour(): Segment | null {
	 ...
	 * @returns упомянутый отрезок.
	 */
	private getSecondSameOrientedNeighbour(): Segment | null {
	...
	public getMiddle(): Point {
		return new Point(
			(this.firstEnd.x + this.secondEnd.x) / 2,
			(this.firstEnd.y + this.secondEnd.y) / 2
		)
	}
	 ...
	 */
	public getIntersectionWithFirstNeighbour(): Point | null {
		if (this.firstNeighbour == null) {
			return null;
	 ...
	 * @returns упомянутая точка.
	 */
	public getIntersectionWithSecondNeighbour(): Point | null {
	 ...
	 * @returns упомянутая точка.
	 */
	public getRatioPoint(ratio: number): Point {
		if (ratio == 0) {
			return this.firstEnd
		} else if (ratio == 1) {
			return this.secondEnd
