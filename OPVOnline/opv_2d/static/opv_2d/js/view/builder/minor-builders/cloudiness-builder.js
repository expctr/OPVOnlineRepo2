...
class CloudinessBuilder {
    ...
    build(cloudinessImage, mltRotationAngle, visualizationDisplay, imageOnloadListener) {
        this.graphicalComposite = new GraphicalComposite();
        CoordinateAdapter.firstCoordinateAdapter.setDisplay(visualizationDisplay);
        CoordinateAdapter.firstCoordinateAdapter.set(-visualizationDisplay.getWidth() / 2, visualizationDisplay.getHeight() / 2, CoordinateSystem.DISPLAY_NOT_ROTATED);
        let rotationAngleInRadians = mltRotationAngle / 24 * 2 * Math.PI;
        this.graphicalComposite.add(new ImageObject(cloudinessImage, CoordinateAdapter.firstCoordinateAdapter.getX(CoordinateSystem.CANVAS), CoordinateAdapter.firstCoordinateAdapter.getY(CoordinateSystem.CANVAS), visualizationDisplay.getWidth(), visualizationDisplay.getHeight(), rotationAngleInRadians, imageOnloadListener));
    }
...
CloudinessBuilder.cloudinessBuilder = new CloudinessBuilder();
